...
if torch.cuda.is_available():
    torch.cuda.set_device(2)  # Use GPU 2 (Tesla T4)
...
# Base class for all TL models
class TLBase(nn.Module):
    def __init__(self):
        super(TLBase, self).__init__()
        self.layer_norm = nn.LayerNorm(1)  # Add LayerNorm
        ...
        for param in self.parameters():
            if isinstance(param, nn.Parameter):
                if param.dim() > 1:
                    ...
                    # For 1D parameters (biases, etc), initialize with small random values
                    nn.init.uniform_(param, -0.1, 0.1)
                    
    def normalize_input(self, x):
        """Apply layer normalization to input"""
        if isinstance(x, torch.Tensor):
            # Add a dimension for LayerNorm if needed
            if x.dim() == 2:
                x = x.unsqueeze(-1)
...
class TL_NN1(TLBase):
    def __init__(self, T):
        ...
        self.t1 = torch.nn.Parameter(torch.randn(1,T) * 0.1, requires_grad=True)
        self.t1_2 = torch.nn.Parameter(torch.randn(1,T) * 0.1, requires_grad=True)
        self.t2 = torch.nn.Parameter(torch.randn(1,T) * 0.1, requires_grad=True)
        self.t2_2 = torch.nn.Parameter(torch.randn(1,T) * 0.1, requires_grad=True)
        self.t3 = torch.nn.Parameter(torch.randn(1,T) * 0.1, requires_grad=True)
        self.t3_2 = torch.nn.Parameter(torch.randn(1,T) * 0.1, requires_grad=True)
        self.b1 = torch.nn.Parameter(torch.randn(1,T) * 0.1, requires_grad=True)
        self.b1_2 = torch.nn.Parameter(torch.randn(1,T) * 0.1, requires_grad=True)
        self.b2 = torch.nn.Parameter(torch.randn(1,T) * 0.1, requires_grad=True)
        self.b2_2 = torch.nn.Parameter(torch.randn(1,T) * 0.1, requires_grad=True)
        self.b3 = torch.nn.Parameter(torch.randn(1,T) * 0.1, requires_grad=True)
        self.b3_2 = torch.nn.Parameter(torch.randn(1,T) * 0.1, requires_grad=True)
        self.A1 = torch.nn.Parameter(torch.rand(1,T) * 0.1,requires_grad=True)
        self.A2 = torch.nn.Parameter(torch.rand(1,T) * 0.1,requires_grad=True)
        self.A3 = torch.nn.Parameter(torch.rand(1,T) * 0.1,requires_grad=True)
        self.A4 = torch.nn.Parameter(torch.rand(1,3) * 0.1,requires_grad=True)
